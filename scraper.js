...
Scraper.prototype.move = function(controls) {
	let bp = this.pos.clone();
	this.onGround = false;
	for (let i = 0; i < 8; ++i) {
		this.pos.addeq(this.vel.mul(.125));
		let ds = [
			cD(this.pos.x + (this.faceLeft ? 18 : 22), this.pos.y + 52.5, 7.5),
			cD(this.pos.x + (this.faceLeft ? 10 : 30), this.pos.y + 30, 7.5),
		];
...
Scraper.prototype.draw = function(ctx) {
